...
from log import Log
log = Log(__name__, log_cate="results_0623-2.5" ).getlog()
...
for i in range (1,11):
    ...
    log.info("Iteration: %d; Configuration: grid: %d, safety_threshold: %f, privacy_threshold: %f, the starting point: [%d, %d, %d]; the end point: [%d, %d, %d]; T_budget(alpha): "
             "%f (%f); "
             "T_optimal(beta): %f (%f); Exploration_rate: %f"
             %(iteration, grid_x, safety_threshold, privacy_threshold, x1, y1, z1, x2, y2, z2, T_budget, alpha, T_optimal, beta, exploration_rate) )
    SaveMap(config, iteration, exploration_rate)
