...
def partition(the_list, first, last):
    pivot_value = the_list[first].g + the_list[first].h
    left_mark = first + 1
    right_mark = last

    done = False
    while not done:
        while left_mark <= right_mark and (the_list[left_mark].g + the_list[left_mark].h) >= pivot_value:
            left_mark += 1

        while left_mark <= right_mark and (the_list[right_mark].g + the_list[right_mark].h) <= pivot_value:
            right_mark -= 1
